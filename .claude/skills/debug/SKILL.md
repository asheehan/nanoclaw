...
**Important:** The docker runs as user `node` with `HOME=/home/node`. Session files must be mounted to `/home/node/.claude/` (not `/root/.claude/`) for session resumption to work.
...
**Docker mount syntax:**
- `-v` syntax supports `:ro` suffix for readonly mounts:
  ```bash
  # Readonly
...
The docker runs as user `node` (uid 1000). Check ownership:
```bash
...
echo -e "\n3. Docker running?"
docker info &>/dev/null && echo "OK" || echo "NOT RUNNING - start Docker (sudo systemctl start docker on Linux, or start Docker Desktop on macOS)"
