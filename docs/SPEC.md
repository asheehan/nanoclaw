...
NanoClaw runs as a system service (launchd on macOS, systemd on Linux).

### Startup Sequence

When NanoClaw starts, it:
1. **Ensures Docker is running** - Checks that the Docker daemon is available
...
| "Claude Code process exited with code 1" | Docker not running | Check `docker info`; ensure Docker daemon is running |
